...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  switch (command) {
    ...
    case ANALOG_MAPPING_QUERY:
      ...
      break;
    case 0x05:
      int8_t enablePin = argv[0];
      int8_t dirPin = argv[1];
      int8_t stepPin = argv[2];
      int8_t timesToMove = 7;
      if(argc == 4) {
        timesToMove = argv[3];  
      }
      ...
      for(int16_t times = 0;times < timesToMove;times++) {
        digitalWrite(dirPin, currentDir); // Set Dir high
        currentDir = HIGH - currentDir; //change dir
        for(int16_t x = 0; x < 40; x++) // Loop 200 times
        {
          digitalWrite(stepPin,LOW); // Output high
          delay(1); // Wait
